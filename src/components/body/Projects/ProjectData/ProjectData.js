export const ProjectData = [
    {
        id: 1,
        name: "YelpCamp",
        details: "A blogging website where authenticated users can add or update destinations of their choice. Also they can review on destinations added by others.",
        code: "https://github.com/Yatnesh1410/YelpCamp",
        tags: ["NodeJS" , "ExpressJS" , "MongoDB", "Heroku"],
        image: require("../../../../assets/yelpcamp.png"),
    },

    {
        id: 2,
        name: "Alan-AI News Application",
        details: "A news app which provides us with news about different topics or from different sources as per our preference, controlled by ALAN-AI voice assisstant.",
        code: "https://github.com/Yatnesh1410/VoicePoweredNewsApp",
        tags: ["ReactJS" , "Alan-AI", "Material-UI", "Netlify"],
        image: require("../../../../assets/news-app.png"),
    },

    {
        id: 3,
        name: "Covid-19 Tracker",
        details: "A single page Web-Application showcasing vital Covid-19 stats from all over the world and further categorize it by country.",
        code: "https://github.com/Yatnesh1410/Covid_19-Tracker",
        tags: ["ReactJS","REST API", "Netlify"],
        image: require("../../../../assets/covidtracker.jpeg"),
    },

    {
        id: 4,
        name: "Real Estate Web-App",
        details: "A simple Website which shows properties available for renting or buying , filtered by various factors such as number of rooms, area or location.",
        code: "https://github.com/Yatnesh1410/Real_Estate",
        tags: ["ReactJS" , "NextJS" ,"Chakra-UI", "Rapid API", "Vercel"],
        image: require("../../../../assets/real_estate.png"),
    },

    {
        id: 5,
        name: "Web-Searcher",
        details: "A responsive app which gives up-to-date results, news, images and videos with ability to switch between light and dark mode.",
        code: "https://github.com/Yatnesh1410/SearcherWebApp",
        tags: ["ReactJS" , "TailWind-CSS" , "Rapid API" , "Netlify"],
        image: require("../../../../assets/searcher.png"),
    },

    {
        id: 6,
        name: "Expense Tracker",
        details: "A simple React App which can help to maintain a record of our expenses on different budgets. We can add or remove budget section according to our convinience.",
        code: "https://github.com/Yatnesh1410/ExpenseTracker",
        tags: ["ReactJS" , "React-Bootstrap" , "Netlify"],
        image: require("../../../../assets/expense-tracker.png"),
    }
];